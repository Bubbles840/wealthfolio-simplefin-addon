...
export function SyncPage({ ctx, store, onReset, scheduler }: Props) {
  ...
  useEffect(() => {
    ...
    ]).then(([last, m, names, bal, catalog, wfAccounts, cards, lastImported, amazon, ui, companion]) => {
      setLastSyncAt(last);
  ...
  return (
    <div className="sfin-page">
      ...
      {/* Everything below here is configured once and then only checked, plus
          the destructive reset flow at the bottom. Rendered inline for now:
          the tab bar arrives in a later task. */}
      <AdvancedTab
        ctx={ctx}
        store={store}
        scheduler={scheduler}
